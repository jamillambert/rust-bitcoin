...
fn segwit_block_test() {
    ...
    let work = bitcoin::pow::Work::from(0x257c3becdacc64_u64);

    assert!(decode.is_ok());
    let real_decode = decode.unwrap();
    assert_eq!(real_decode.header.version, Version::from_consensus(Version::USE_VERSION_BITS as i32)); // VERSIONBITS but no bits set
